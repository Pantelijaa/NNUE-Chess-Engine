#!/usr/bin/env python3
"""
Reference: https://github.com/official-stockfish/nnue-pytorch/blob/master/scripts/easy_train.py
"""
import time
import sys
import psutil
...
def is_nnue_pytorch_setup(directory):
    try:
        # data_loader is a package with relative imports, so it must be imported
        # as a module (running __init__.py as a script fails on `from .config ...`).
        with subprocess.Popen(
            [sys.executable, "-c", "import data_loader"], cwd=directory
        ) as process:
            if process.wait(timeout=TIMEOUT):
                return False
            return True
...
def setup_nnue_pytorch(directory, repo, branch_or_commit):
    ...
    # Windows/mingw build fixes for the data loader. The tree is re-extracted from
    # a zip on every setup, so re-apply these to the CMake flags each time:
    #  1) parallel_dataloader.h calls POSIX gmtime_r, which mingw-w64's <time.h>
    #     only declares when _POSIX_THREAD_SAFE_FUNCTIONS is defined.
    #  2) PGO build: the profile-generate exe links the profile-generate shared
    #     lib, so the gcov runtime is defined twice -> multiple definition. The
    #     accepted mingw workaround is -Wl,--allow-multiple-definition.
    #  3) The built .dll depends on libstdc++-6/libgcc_s_seh-1/libwinpthread-1.
    #     Python 3.8+ ignores PATH when resolving a ctypes DLL's dependencies, so
    #     ctypes can't find them. Statically link the mingw runtime (-static ...)
    #     so the loader .dll is self-contained.
    if sys.platform == "win32":
        cmakelists = os.path.join(directory, "data_loader", "cpp", "CMakeLists.txt")
        try:
            with open(cmakelists, "r", encoding="utf-8") as f:
                text = f.read()
            needle = 'set(CMAKE_CXX_FLAGS "-march=native -Wall")'
            link_extra = "-Wl,--allow-multiple-definition -static -static-libgcc -static-libstdc++"
            repl = (
                'set(CMAKE_CXX_FLAGS "-march=native -Wall -D_POSIX_THREAD_SAFE_FUNCTIONS")\n'
                f'set(CMAKE_EXE_LINKER_FLAGS "${{CMAKE_EXE_LINKER_FLAGS}} {link_extra}")\n'
                f'set(CMAKE_SHARED_LINKER_FLAGS "${{CMAKE_SHARED_LINKER_FLAGS}} {link_extra}")'
            )
            if needle in text and "_POSIX_THREAD_SAFE_FUNCTIONS" not in text:
                with open(cmakelists, "w", encoding="utf-8") as f:
                    f.write(text.replace(needle, repl))
                LOGGER.info("Patched data loader CMakeLists for mingw (gmtime_r + PGO link).")
        except Exception as e:
            LOGGER.warning(f"Could not patch data loader CMakeLists: {e}")
